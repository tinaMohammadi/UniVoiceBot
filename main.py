...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = "🚀 سلام! به ربات تعاملی خوش آمدید.\nبرای شروع یکی از گزینه‌های زیر را انتخاب کنید:"
    if update.message:
        await update.message.reply_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
    else:
        await update.callback_query.answer()
...
async def delete_form(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        await query.answer()
        await query.message.edit_text("❌ فرم حذف شد. برای شروع دوباره /start بزنید.")
    return ConversationHandler.END

async def generic_step(update: Update, context: ContextTypes.DEFAULT_TYPE, key, next_q, next_state):
    context.user_data[key] = update.message.text
    await update.message.reply_text(next_q, parse_mode="Markdown")
    return next_state

# Stages mapping
async def ask_course(u, c): return await generic_step(u, c, "استاد", "*📚 درس:*", ASK_COURSE)
async def ask_teaching(u, c): return await generic_step(u, c, "درس", "*🎓 نوع تدریس:*", ASK_TEACHING)
async def ask_ethics(u, c): return await generic_step(u, c, "نوع تدریس", "*💬 خصوصیات اخلاقی:*", ASK_ETHICS)
async def ask_notes(u, c): return await generic_step(u, c, "خصوصیات اخلاقی", "*📄 جزوه:*", ASK_NOTES)
async def ask_project(u, c): return await generic_step(u, c, "جزوه", "*🧪 پروژه:*", ASK_PROJECT)
async def ask_attend(u, c): return await generic_step(u, c, "پروژه", "*🕒 حضور و غیاب:*", ASK_ATTEND)
async def ask_midterm(u, c): return await generic_step(u, c, "حضور و غیاب", "*📝 میان‌ترم:*", ASK_MIDTERM)
async def ask_final(u, c): return await generic_step(u, c, "میان‌ترم", "*📘 پایان‌ترم:*", ASK_FINAL)
async def ask_match(u, c): return await generic_step(u, c, "پایان‌ترم", "*📊 تطبیق با جزوه (از ۵):*", ASK_MATCH)
async def ask_contact(u, c): return await generic_step(u, c, "تطبیق سوالات", "*📞 راه ارتباطی:*", ASK_CONTACT)
async def ask_conclusion(u, c): return await generic_step(u, c, "راه ارتباطی", "*📌 نتیجه‌گیری:*", ASK_CONCLUSION)
async def ask_semester(u, c): return await generic_step(u, c, "نتیجه‌گیری", "*📅 ترم:*", ASK_SEMESTER)
async def ask_grade(u, c): return await generic_step(u, c, "ترم", "*⭐️ نمره از ۲۰:*", ASK_GRADE)
...
async def submit_form(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    summary = build_form_text(context.user_data)
    keyboard = [[InlineKeyboardButton("✅ تایید", callback_data=f"admin_accept:{query.from_user.id}"),
                 InlineKeyboardButton("❌ رد", callback_data=f"admin_reject:{query.from_user.id}")]]
    await context.bot.send_message(chat_id=ADMIN_ID, text=f"📥 فرم جدید:\n\n{summary}", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
    await query.message.edit_text("📨 فرم برای ادمین ارسال شد.")
...
async def admin_actions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "admin_accept":
        msg = await context.bot.send_message(chat_id=CHANNEL_ID, text=query.message.text.split("📥 فرم جدید:")[1], parse_mode="Markdown")
        post_reactions[msg.message_id] = {"likes": set(), "dislikes": set()}
        await msg.edit_reply_markup(reply_markup=reaction_keyboard(msg.message_id))
        await context.bot.send_message(chat_id=user_id, text="✅ فرم شما منتشر شد.")
    else:
...
async def anon_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.message.reply_text("🕵️ حالت ناشناس فعال شد. پیام بده! (برای خروج دکمه پایان چت در پیام‌های بعدی را بزن)")
...
async def end_chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == ADMIN_ID and user_id in reply_sessions:
        target = reply_sessions[user_id]
        if target in active_chats: del active_chats[target]
        await context.bot.send_message(chat_id=target, text="🔚 چت توسط ادمین بسته شد.")
        del reply_sessions[user_id]
    await query.message.reply_text("✅ چت پایان یافت.")

async def receive_msg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.message.from_user
    u_id = user.id
    
    if u_id == ADMIN_ID and u_id in reply_sessions:
        target_id = reply_sessions[u_id]
        kb = [[InlineKeyboardButton("❌ پایان چت", callback_data="end_chat")]]
    ...
    if active_chats.get(u_id):
        kb = [[InlineKeyboardButton("✉️ پاسخ", callback_data=f"reply_to:{u_id}"), InlineKeyboardButton("❌ پایان", callback_data="end_chat")]]
        info = f"👤 {user.full_name} | `{u_id}`\n\n{update.message.text}"
        await context.bot.send_message(chat_id=ADMIN_ID, text=info, reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown")
        await update.message.reply_text("✅ فرستادم.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ پایان چت", callback_data="end_chat")]]))

async def admin_reply_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    reply_sessions[ADMIN_ID] = int(update.callback_query.data.split(":")[1])
    await update.callback_query.message.reply_text("✍️ بنویس تا براش بفرستم:")

async def handle_reaction(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    act, mid = query.data.split(":")
    mid = int(mid)
    res = post_reactions.setdefault(mid, {"likes": set(), "dislikes": set()})
    if act == "like":
        res["dislikes"].discard(query.from_user.id); res["likes"].add(query.from_user.id)
    else:
        res["likes"].discard(query.from_user.id); res["dislikes"].add(query.from_user.id)
    await query.message.edit_reply_markup(reply_markup=reaction_keyboard(mid))
